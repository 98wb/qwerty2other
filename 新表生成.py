...
def py_cont(txt_name):
    ...
    while True:
        ...
        for key in codes:
            #print("key is : %s"% key )
            for word in key:
                cur.execute("SELECT A_key FROM sing_dic WHERE aim_chars = '%s'" % word)
                b = cur.fetchone()
    ...
    while True:
        ...
        if len(cut_line) == 4:
            ...
            for rk_w in rk:
                ...
                for kword in rk_w:
                    cur.execute("SELECT A_key FROM sing_dic WHERE aim_chars = '%s'" % kword)
                    bk = cur.fetchone()
...
while True:
  ...
  if len(py_line) == 1:
      ...
      new_table.write(py_line[0]+'\t'+ py_line[1].strip() +'\t'+py_line[2]+'\n')
  if conf_line == "":
      break
rd_conf.close()
new_table.close()
